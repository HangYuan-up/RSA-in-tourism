...
behavior_PL<-timedata[1,c(16:21)]
behavior_PL<-as.data.frame(behavior_PL)
behavior_PL<-as.matrix(dist(t(behavior_PL)))
behavior_PL<-1-behavior_PL/max(behavior_PL)

for (i in 1:601){
  PL<-timedataslice[i,c(3:8)]
  PL<-as.data.frame(PL)
  PL<-as.matrix(dist(t(PL)))
  PL<-1-PL/max(PL)
  a<-mantel.rtest(dist(t(PL)),dist(t(behavior_PL)),nrepet = 999)
  correlation[i]=a[["obs"]]
  pvalue[i]=a[["pvalue"]]
  if (pvalue[i]<=0.05){
    significance[i]=1}
