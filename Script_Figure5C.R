...
library(ggalt)
library(ade4)
#Figure 5C:Time-point-wise representational similarity values (r) between neural activity RDM and theoretical RDM for participants in the OT group.
timedata <- read_excel(".../Dataset_Figure5_timeserisedata_bio.xlsx")
...
for (i in 1:601){
  ...
  OT<-1-OT/max(OT)
  a<-mantel.rtest(dist(t(OT)),dist(t(concept)),nrepet = 999)
  correlation[i]=a[["obs"]]
  pvalue[i]=a[["pvalue"]]
  if (pvalue[i]<=0.05){
    significance[i]=1}
